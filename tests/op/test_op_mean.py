import numpy as np
import auto_diff as ad
from .util import NumGradCheck


class TestOpMean(NumGradCheck):

    def test_forward(self):
        val = np.random.random((3, 5))
        w = ad.array(val)
        y = w.transpose().mean()
        actual = y.forward()
        expect = np.mean(val)
        self.assertEqual((), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))
        y = w.transpose().mean(axis=-1)
        actual = y.forward()
        expect = np.mean(val, axis=0)
        self.assertEqual((5,), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))
        y = w.transpose().mean(axis=0)
        actual = y.forward()
        expect = np.mean(val, axis=-1)
        self.assertEqual((3,), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))
        y = w.transpose().mean(axis=(0, -1))
        actual = y.forward()
        expect = np.mean(val)
        self.assertEqual((), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))

    def test_forward_keepdims(self):
        val = np.random.random((3, 5))
        w = ad.array(val)
        y = ad.mean(w.transpose(), keepdims=True)
        actual = y.forward()
        expect = np.mean(val, keepdims=True)
        self.assertEqual((1, 1), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))
        y = w.transpose().mean(axis=-1, keepdims=True)
        actual = y.forward()
        expect = np.transpose(np.mean(val, axis=0, keepdims=True))
        self.assertEqual((5, 1), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))
        y = w.transpose().mean(axis=0, keepdims=True)
        actual = y.forward()
        expect = np.transpose(np.mean(val, axis=-1, keepdims=True))
        self.assertEqual((1, 3), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))
        y = w.transpose().mean(axis=(0, -1), keepdims=True)
        actual = y.forward()
        expect = np.mean(val, keepdims=True)
        self.assertEqual((1, 1), y.shape)
        self.assertTrue(np.allclose(expect, actual), (expect, actual))

    def test_backward(self):
        val = np.random.random((3, 5))
        w = ad.variable(val, name='W')
        y = w.transpose().mean()
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=-1)
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=0)
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=(0, -1))
        self.numeric_gradient_check(y, {}, [w])
        val = np.random.random((3, 4, 5))
        w = ad.variable(val, name='W')
        y = w.transpose().mean()
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=(0, 2)).mean(axis=0)
        self.numeric_gradient_check(y, {}, [w])

    def test_backward_keepdims(self):
        val = np.random.random((3, 5))
        w = ad.variable(val, name='W')
        y = ad.mean(w.transpose(), keepdims=True)
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=-1, keepdims=True)
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=0, keepdims=True)
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=(0, -1), keepdims=True)
        self.numeric_gradient_check(y, {}, [w])
        val = np.random.random((3, 4, 5))
        w = ad.variable(val, name='W')
        y = w.transpose().mean(keepdims=True)
        self.numeric_gradient_check(y, {}, [w])
        y = w.transpose().mean(axis=(0, 2), keepdims=True).mean(axis=1, keepdims=True)
        self.numeric_gradient_check(y, {}, [w])

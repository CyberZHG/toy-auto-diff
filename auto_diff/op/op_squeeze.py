...
class OpSqueeze(Operation):
    ...
    def _get_name(self) -> str:
        if self.axis == -1:
            return 'squeeze(%s)' % self.x.name
        return 'squeeze(%s, axis=%s)' % (self.x.name, str(self.axis))

    def _get_op_name(self) -> str:
        if self.axis == -1:
            return 'squeeze(%s)' % self.x._op_name
        return 'squeeze(%s, axis=%s)' % (self.x._op_name, str(self.axis))

    def _forward(self, feed_dict: Mapping[Union[str, OpPlaceholder], np.ndarray]) -> np.ndarray:
        """Squeeze the tensor."""
        return self.x.forward(feed_dict).squeeze(axis=self.axis)

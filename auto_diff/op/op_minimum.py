...
class OpMinimum(Operation):
    """Element-wise minimum."""

    def __init__(self, x: Operation, y: Operation, **kwargs):
        self.inputs = [x, y]
        self._broadcast_shape(x, y)
        super(OpMinimum, self).__init__(**kwargs)

    def _get_name(self) -> str:
        return 'minimum(%s, %s)' % (self.inputs[0].name, self.inputs[1].name)

    def _get_op_name(self) -> str:
        return 'minimum(%s, %s)' % (self.inputs[0]._op_name, self.inputs[1]._op_name)

    def _forward(self, feed_dict: Mapping[Union[str, OpPlaceholder], np.ndarray]) -> np.ndarray:
        return np.minimum(self.inputs[0].forward(feed_dict), self.inputs[1].forward(feed_dict))

...
class OpExpandDims(Operation):
    ...
    def _get_name(self) -> str:
        if self.axis == -1:
            return 'expand_dims(%s)' % self.x.name
        return 'expand_dims(%s, axis=%d)' % (self.x.name, self.axis)

    def _get_op_name(self) -> str:
        if self.axis == -1:
            return 'expand_dims(%s)' % self.x._op_name
        return 'expand_dims(%s, axis=%d)' % (self.x._op_name, self.axis)

    def _forward(self, feed_dict: Mapping[Union[str, OpPlaceholder], np.ndarray]) -> np.ndarray:
        """Expand shape."""
        return np.expand_dims(self.x.forward(feed_dict), axis=self.axis)
